...
class Trainer(object):
    ...
    def _build_optimizer(self):
        if '_alr' in self.args.xlmr_task:
            
            layer_lr = self.args.layerwise_lr 
            params=[{'params':[],'lr':layer_lr[i]} for i in range(13)]
            for name, param in self.model.named_parameters():
                if param.requires_grad == False:
                    continue
                if name.startswith('encoder.xlmr.model.encoder.sentence_encoder.layers.'):
                    layer_id = int(name.split('.')[6])
                    params[layer_id]['params'] += [param]   
                else:
                    params[-1]['params'] += [param]
        else:
            params = list(
                filter(
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try: 
                state = checkpoint_utils.load_checkpoint_to_cpu(filename, args=self.args)
            ...
            if "metrics" in extra_state and not reset_meters:
                ...
                for meter in metrics.get_meters("default"):
                    if isinstance(meter, meters.TimeMeter):
                        meter.reset()
        else:  ## means no checkpoint_last.pt file.
            if  self.args.xlmr_task == 'embnoft_ms' or '_2stage' in self.args.xlmr_task:    
                f_checkpoint_path = os.path.join(self.args.ft_last_f_dir, "checkpoint_best.pt") 
                ...
                base_dict = {k:v for k,v in f_state['model'].items() if (k in model_f_dict) and ('xlmr' not in k) and ('encoder.embed_tokens' not in k)}
