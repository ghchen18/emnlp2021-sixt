...
class TranslationTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--fixed-dictionary",help="Fixed dictionary to use with model path",
        ...
        ### option for denoising dataset
        parser.add_argument(
            '--mask', default=0.15, type=float,
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if self.args.eval_bleu:
            ...
            def sum_logs(key):
                mx = sum(log.get(key, 0) for log in logging_outputs)
                if hasattr(mx, 'device'):
                    mx = mx.cpu()
                return mx
