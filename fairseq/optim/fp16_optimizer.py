...
class _FP16OptimizerMixin(object):
    ...
    def build_fp32_params(cls, params, flatten=True):
        ...
        if flatten:
            ...
            # layer_base_size = sum(p.data.numel() for p in params[:4])
            # layer_size = sum(p.data.numel() for p in params[20:36])
            ...
            for px in params:
                if type(px) is dict:
                    p32 = {}
                    p32['lr'] = px['lr']
                    p32['params'] = [torch.nn.Parameter(x.data.float()) for x in px['params'] ]                    
                else:
                    p32 = torch.nn.Parameter(px.data.float())
    ...
    def zero_grad(self):
        ...
        for px in self.fp16_params:
            if type(px) is dict:
                for item in px['params']:
                    item.grad = None
            else:
                px.grad = None
        if self.has_flat_params:
